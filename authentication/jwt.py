...
SECRET_KEY = "mysecretkey"  # в реальной жизни в bash 'openssl rand -hex 32'
ALGORITHM = "HS256"
...
def create_jwt_token(data: dict, expires_delta: datetime.timedelta | None = None):
    to_encode = data.copy()
    if expires_delta:
        expire = datetime.datetime.now(tz=datetime.timezone.utc) + datetime.timedelta(minutes=120)
...
def get_user_from_token(token: str = Depends(oauth2_scheme)):
    try:
        ...
        return payload.get("sub")  # тут мы идем в полезную нагрузку JWT-токена и возвращаем утверждение
        # о юзере (subject); обычно там еще можно взять "iss" - issuer/эмитент,
...
@app.post("/token/")
async def login(user_data: OAuth2PasswordRequestForm = Depends()):  # тут логинимся через форму
    ...
    return {
        "access_token": create_jwt_token({"sub": user_data.username}, access_token_expires)
